...
4. Select whether you want your plot to include outliers in the API data. Lower outlier threshold is calculated as 25% quartile(data) - 1.5*IQR(data)`; upper outlier threshold calculated as `75% quartile(data) + 1.5*IQR(data). **Default is to include outliers.** You can change this by changing `outliers = False` to `outliers = True` in `tools/figs.py`. The plots are interactive, so you can zoom in to rescale and more closely examine your data, if outliers in the API data are overly influencing the scale of the plot. 
